...
def create_decoder_input(dataset,pred_len=1,encoder_input_len=15,rag=1):
    decoder_input_data=[]
    for i in range(len(dataset)-encoder_input_len-pred_len-rag+1):
        add_array = [dataset[i+j+rag+encoder_input_len,1] for j in range(pred_len)]
...
def create_dataset(input_len, fit_len, region):
    ...
    train_encoder_input_data=create_encoder_input(train_ds,encoder_input_len=input_len,pred_len=fit_len)
    train_target_data=create_target_data(train_ds,encoder_input_len=input_len,pred_len=fit_len)
    train_decoder_input_data=create_decoder_input(train_ds,encoder_input_len=input_len,pred_len=fit_len)

    test_encoder_input_data=create_encoder_input(test_ds,encoder_input_len=input_len,pred_len=fit_len)
    test_target_data=create_target_data(test_ds,encoder_input_len=input_len,pred_len=fit_len)
    test_decoder_input_data=create_decoder_input(test_ds,encoder_input_len=input_len,pred_len=fit_len)

    train_dataset = FluDataset(train_encoder_input_data, train_decoder_input_data, train_target_data, transform)
    test_dataset = FluDataset(test_encoder_input_data, test_decoder_input_data, test_target_data)
